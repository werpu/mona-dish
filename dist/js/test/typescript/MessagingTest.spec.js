...
import { BroadcastChannelBroker, BroadcastChannelBrokerBuilder, Broker, Message } from "../../main/typescript/Messaging";
...
let iframe = `
        ...
        `;
const CHANNEL = "booga";
let applyMessageReceiver = function (contentWindow, msg, brokr = new Broker(contentWindow)) {
...
<div id="shadow1"></div>
    ...
    it('from iframe to global', function () {
        let contentWindow = (global.document.getElementById('iframe1')).contentWindow;
        ...
        origBroker.registerListener(CHANNEL, (msg) => {
    ...
    it('basic init', function () {
        var _a;
        expect((_a = window === null || window === void 0 ? void 0 : window.document) === null || _a === void 0 ? void 0 : _a.body).not.eq(null);
    });
    ...
    it("must work with Broadcast channel", function (done) {
        let broker1 = new BroadcastChannelBroker((group) => new BC(group));
        let broker2 = new BroadcastChannelBroker((group) => new BC(group));
        let brokerReceived = 0;
        new Promise((apply) => {
            broker2.registerListener(CHANNEL, () => {
                brokerReceived++;
